...
# 
# grid <- expand.grid(seq(0,1,by = 0.01), seq(0,1,by = 0.01), seq(0,1,by = 0.01), seq(0,1,by = 0.01))
# grid[rowSums()]
# tail(grid)
...
gridd <- expand.grid(seq(0, 1, by = 0.01), seq(0, 0.1, by = 0.01), seq(0, 1, by = 0.01))
...
for(i in 1:1){
  ...
  for (j in 1:nrow(gridd)){
    ...
    mdat <- data.frame(month = i+2,
                       ...
                       ABL_threshold = nspray[1],
                       ABD_threshold = nspray[2],
                       CD_threshold = nspray[3],
                       NI_threshold = nspray[4])
